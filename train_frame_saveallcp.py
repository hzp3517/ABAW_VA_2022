...
def eval(model, val_iter):
    ...
    if model.task == 'v' or model.task == 'a':
        ...
        for i, data in enumerate(val_iter):  # inner loop within one epoch
            ...
            model.test()
            pred = model.output.detach().cpu().numpy() # (bs,)
            label = data['label'].numpy() # (bs, 2)
            if model.task == 'v':
                label = label[:, 0]
            else:
                label = label[:, 1]
        ...
        for i, data in enumerate(val_iter):  # inner loop within one epoch
            model.set_input(data)         # unpack data from dataset and apply preprocessing
            model.test()
            pred = model.output.detach().cpu().numpy() # (bs, 2)
            label = data['label'].numpy() # (256, 2)

            pred = pred.transpose() # (2, bs)
            label = label.transpose() # (2, 256)

            valence_total_pred.append(pred[0])
            valence_total_label.append(label[0])
            arousal_total_pred.append(pred[1])
            arousal_total_label.append(label[1])
