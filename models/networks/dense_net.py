...
class DenseNetEncoder(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(16, 16, 16), num_init_features=24,
        ...
        # # params initialization # 因为有可能要微调，所以这里不要初始化权重，权重统一到外面的主模型中处理
        # for m in self.modules():
        #     if isinstance(m, nn.Conv2d):
        #         nn.init.kaiming_normal_(m.weight)
        #     elif isinstance(m, nn.BatchNorm2d):
    ...
    def forward(self, images):
        '''
        input-shape: (batch, channel, heigh, width)
        '''
...
if __name__ == '__main__':
    ...
    # model = DenseNet(gpu_id=gpu_id)
    # input = torch.rand((8, 64, 64, 1)).to(device) # (batch, heigh, width, channel)
    # batch = {'images': input}
    ...
    input = torch.rand((8, 1, 64, 64)) # (batch, channel, heigh, width)
    ft = model(input) # (8, 342)
    print(model)
    print(ft.shape)
