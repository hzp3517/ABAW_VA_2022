'''
https://github.com/yaohungt/Multimodal-Transformer/blob/HEAD/modules/transformer.py#L4
'''
...
class TransformerEncoder_attn(nn.Module):
    """
...
if __name__ == '__main__':
    net = TransformerEncoder_attn(embed_dim=256, num_heads=4, layers=4) # 对应于nn.TransformerEncoder的参数dim_feedforward，在模型中已经设定为4*embed_dim
    inp = torch.rand(2, 75, 256) # (bs, seq_len, embd_dim)
    inp = inp.transpose(0, 1) # (seq_len, bs, embd_dim)
    hidden_states = net(inp) # (seq_len, bs, embd_dim)
    hidden_states = hidden_states.transpose(0, 1)
    print(hidden_states.shape) # (bs, seq_len, embd_dim)