...
class TransformerModel(BaseModel):
    ...
    #     if load_label:
    #         if self.target_name == 'both':
    #             self.target = torch.stack([input['valence'], input['arousal']], dim=2)
    #         else:
    #             self.target = input[self.target_name]

    #         if self.cls_loss:
    #             if self.target_name == 'both':
    #                 #[B, L, 2]
    #                 self.cls_target = torch.stack([input['valence_cls'], input['arousal_cls']], dim=2)
    #             else:
    #                 self.cls_target = input[self.target_name]
    ...
    #     # mems = None
    #     for step in range(split_seg_num):
    #         feature_step = self.feature[:, step*self.max_seq_len: (step+1)*self.max_seq_len].to(self.device)
    #         mask = self.mask[:, step*self.max_seq_len: (step+1)*self.max_seq_len].to(self.device)
    #         # if self.encoder_type == 'transformer_xl':
    #         #     prediction, logits, mems = self._forward_step(feature_step, mems)
    #         # else:
    #         #     prediction, logits = self.forward_step(feature_step, mask)
    ...
    #         if self.isTrain:
    #             self.optimizer.zero_grad()  
    #             target = self.target[:, step*self.max_seq_len: (step+1)*self.max_seq_len].to(self.device)
    #             cls_target = None if not self.cls_loss else self.cls_target[:, step*self.max_seq_len: (step+1)*self.max_seq_len].to(self.device)
    #             self.backward_step(prediction, target, mask, logits, cls_target)
    #             self.optimizer.step() 
    #     self.output = torch.cat(self.output, dim=1)
