...
auto_csv_dir = 'autorun/csv_results/adjust_lr'       # 生成结果csv文件路径，注意train_csv.py文件中也需要同步修改！！！
# 当需要在同一组设定下跑多次时候最好在这里开个子目录，而不是改下面的"name"

task_script = 'scripts/train_lstm.sh'     # 执行script路径
avialable_gpus = [1, 2, 3, 4, 5, 6, 7]                 # 可用GPU有哪些
num_sessions = 7                        # 一共开多少个session同时执行（即开几个screen的会话）
avialable_gpus = avialable_gpus[:num_sessions]
screen_name = 'hzp_abaw_train'
independent_parameters = {                              # 一共有哪些非关联参数
    # bash scripts/train_lstm.sh fclstm-xl valence denseface None 32 1e-4 0.3 128 256,256 100 mse 1 1

    'name': ['fclstm-xl'], #注意：此列表中只能有一个元素，这个名字与log文件名最前面一部分也是关联的
...
param_order_list = ['name', 'target', 'feature', 'norm_features', 'batch_size', 'lr', 'dropout_rate', 'hidden_size', 'regress_layers', 'max_seq_len', 'loss_type', 'run_idx'] #除gpu外所有参数的顺序
