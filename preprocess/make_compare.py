...
def get_compare_ft(audio_root, video_id, num_frames, frame_rate=0.01):
    '''
    num_frames: 30Hz
    '''
    compare = ComParEExtractor(downsample=-1) # 不做降采样，0.01s一帧特征
    ...
    for set_name in set_list:
        ...
        for new_video_id in tqdm(valid_video_list):
            ...
            if valid_h5f[new_video_id]['special'][()] == 0: # 没有被切
                ...
                video_ft = get_compare_ft(audio_root, new_video_id, num_frames)
                video_group['fts'] = video_ft
            else: # 后切出来的片段
                original_video = '_'.join(new_video_id.split('_')[:-1])
                num_frames = original_h5f[original_video]['length'][()]
                video_ft = get_compare_ft(audio_root, original_video, num_frames)
