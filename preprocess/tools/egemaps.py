...
class EgemapsExtractor(object):
    ''' 抽取eGeMAPS特征, 输入音频路径, 输出npy数组, 每帧
    '''
    def __init__(self, opensmile_tool_dir=None, downsample=-1, tmp_dir='.tmp', no_tmp=True):
        ''' Extract eGeMAPS feature
    ...
    def __call__(self, wav):
        ...
        cmd = 'SMILExtract -C {}/config/gemaps/eGeMAPSv01a.conf \
...
if __name__ == '__main__':
    downsample = -1
    no_tmp = False
    compare = EgemapsExtractor(downsample=downsample, no_tmp=no_tmp)
    wav_path = '/data9/hzp/ABAW_VA_2022/processed_data/audios/1-30-1280x720.wav' # 5分45秒
    wav_data = compare(wav_path)
    print(wav_data.shape) # (34517, 23) 有问题，eGeMAPS应该是88维特征  （GeMAPS那个文件抽出来是18维）
    # 0.01s一帧
